...
class LabelFiller(object):
    ...
    async def labelFilter(self, string, labels: dict):
        ...
        for rule in self.rules:
            ...
            if (
                rule.get("type", "") == "avg"
                and rule.get("name", "") not in self.avgvars.keys()
            ):
                self.avgvars[rule.get("name", "")] = avgVar(
                    maxIterations=rule.get("maxIterations", avgVar.maxIterations)
                )

            isFilled = (
                False
                if labels.get(rule.get("name", ""), None) in (False, "", None)
                else True
            )
            ...
            match rule.get("type", ""):
                ...
                case "avg":
                    if bingo:
                        try:
                            result = float(result)
                            labels[rule["name"]] = self.avgvars.get(
                                rule.get("name"), 0
                            ).nextVal(result, rule.get("ndigits", 0))
                        except:
                            pass
...
class avgVar(object):
    ...
    def nextVal(self, aNext, ndigits=0):
        ...
        self.avg = round(
            self.avg * (curI / (curI + 1)) + aNext / (curI + 1), ndigits=ndigits
        )
        if ndigits == 0:

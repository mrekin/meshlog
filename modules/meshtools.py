...
class MeshTools:
    ...
    async def getAvailableMmhSteps(self, platform):
        ...
        for pl in self.boards:
            if pl.get("name", None) == platform:
                ...
                if pl.get("firmwareURL", None) or (
                    pl.get("availableFirmwaresURL", None)
                    and pl.get("baseFirmwareURL", None)
                ):
                    steps.append(mmhSteps.UPDATE_FIRMWARE)
                    if pl.get("availableFirmwaresURL", None) and pl.get(
                        "baseFirmwareURL", None
                    ):
                        fwSelect = True
                break
                ...
                if pl.get("name", None) == platform:
                    ...
                    if not versions:
                        if pl.get("availableFirmwaresURL", None) and pl.get(
                            ...
                            try:
                                with requests.get(
                                    pl.get("availableFirmwaresURL"), timeout=3
                                ) as r:
                                ...
                                self.log.write("Please try to select platform again")
                    return versions
                break
        return []
    ...
    def downloadFile(
        ...
        if response.status_code == 200:
            try:
                ...
                self.log.write("Error while getting filename")
                pass
    ...
    def copyFile(
        ...
        if not url:
            match type:
                case "bootloader":
                    url = self.boards[self.getBoardsList().index(platform)].get(
                        constants.CFG_BOOTLOADER_URL, None
                    )
                    if not url:
                        self.log.write(
                            f"Bootloader url is not set in config ({platform})"
                        )
                        return
                    if not filename:
                ...
                case "fullerase":
                    url = self.boards[self.getBoardsList().index(platform)].get(
                        constants.CFG_FULLERASE_URL, None
                    )
                    if not url:
                        self.log.write(
                            f"Full erase url is not set in config ({platform})"
                        )
                        return
                    if not filename:
                        parsed_url = urlparse(url)
                ...
                case "firmware":
                    if not vars.get("version", None):
                        url = self.boards[self.getBoardsList().index(platform)].get(
                            constants.CFG_FIRMWARE_URL, None
                        )
                    else:
                        url = (
                            self.boards[self.getBoardsList().index(platform)]
                            .get(constants.CFG_BASE_FW_URL, None)
                            .format(**vars)
                        )
                    if not url:
                        self.log.write(
                            f"Firmware url is not set in config ({platform})"
                        )
                        return
                    if not filename:
                        parsed_url = urlparse(url)
    ...
    async def execMmhSteps(
        ...
        if mmhSteps.FULL_ERASE.name in steps:
            ...
            if not bingo:
                self.log.write(
                    "Please enable DFU mode manually (double reset) and do Full Erase again"
                )
            ...
            self.copyFile(
                type="fullerase", platform=platform, targetFolder=targetFolder
            )
            ports = sm.get_available_ports()[0]
            try:
        ...
        if mmhSteps.UPDATE_FIRMWARE.name in steps:
            ...
            if not bingo:
                self.log.write(
                    "Please enable DFU mode manually (double reset) and do UPDATE_FIRMWARE again"
                )
                return stepsExecuted
            self.log.write("Downloading firmware file...")
            self.copyFile(
                type="firmware", platform=platform, targetFolder=targetFolder, vars=vars
            )
            self.log.write("Done")
            stepsExecuted.append(mmhSteps.UPDATE_FIRMWARE.name)

        if mmhSteps.OPEN_CONSOLE.name in steps:
            self.log.write("Opening serial console...")
        ...
        while not os.path.exists(targetFolder) and not res:
            ...
            if newPorts:
                if platform == "nRF52840-nicenano":
                    ...
                    self.log.write(
                        "Done. Check Serial tab. Re-connect USB if Factory restore not catched"
                    )
    ...
    def bootLoaderAvailable(self, platform: str):
        try:
            res = bool(
                self.boards[self.getBoardsList().index(platform)].get(
                    constants.CFG_BOOTLOADER_URL, None
                )
            )
        except Exception:
        ...
        try:
            for port in ports:
                ...
                if port.device not in self.connectedNodes.keys() or overrideExisting:
                    ...
                    try:
                        self.log.write(
                            f"Trying open {port.device} as meshtastic device..."
                        )
                        with meshtastic.serial_interface.SerialInterface(
                            port.device
                        ) as interface:
    ...
    async def saveNodeCfg(self, interface=None, overrideExisting=False):
        ...
        if interface:
            ...
            self.log.write(
                f"Saving node '{nodeId}({nodeInfo.get('user',{}).get('shortName','')})' config..."
            )
            ...
            if not os.path.exists(cfgPath) or overrideExisting:
                ...
                self.log.write(
                    f"Config for {nodeId} already exist. Remove it or save config manually"
                )
        else:
            self.log.write("Node not connected. Connect to a node first.")

    async def openSerial(self, port):
        if self.serialConn is not None and self.serialConn.devPath != port.device:
        ...
        self.log.write("Meshtatic interface opened.")

    async def closeSerial(self):
        if self.serialConn:
            self.log.write("Closing node connection..")
